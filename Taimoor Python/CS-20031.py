...
def matrixMULT():
    print('Important Point : Make sure that for Multiplication of 2 Matrices that Column of Matrix 1 must be Equal to Row of Matrix 2  \n')
...
# CREATING FINAL MATRIX
    if column1==row2:
        ...
        final_matrix=list()
        for i in range(frow):
            values='0 '*column2    # This is creating a n number of 0s to make a row
            val=''
            val+=values                # This is creating a row in final_matrix
            lvalues=val.split()
            final_matrix.append(lvalues)
